...
class KMeans:
    ...
    def optimum(self, X):
        ...
        for K in range(1, self.maxK + 1):
            kmeanModel = k_means(n_clusters=K).fit(X)
            kmeanModel.fit(X)
            distortions.append(sum(np. min(
                cdist(X, kmeanModel.cluster_centers_, 'euclidean'), axis=1)) / X.shape[0])
            inertias.append(kmeanModel.inertia_)

        plt.plot(range(self.maxK), inertias, 'bx-')
        plt.xlabel('Values of K')
        plt.ylabel('inertias')
        plt.title('The Elbow Method using Inertias For Optimal K')
        plt.show()
    ...
    def predict(self, X):
        ...
        for K in range(1, self.maxK):
            ...
            random_sample_idxs = np.random.choice(
                self.n_samples, K, replace=False)
    ...
    def _is_converged(self, centroids_old, centroids, K):
        distances = [euclidean_distance(
            centroids_old[i], centroids[i]) for i in range(K)]
    ...
    def get_J(self, j):
        ...
        for i in range(j):
            for j in range(len(self.clusters[i])):
                sum += np.sqrt((self.X[self.clusters[i]][j][0] - self.centroids[i][0]) ** 2 + (
                    self.X[self.clusters[i]][j][0] - self.centroids[i][1])**2)
        return sum

    def get_D(self, array):
        res = []
        for i in range(1, len(array) - 1):
            res.append(np.abs(array[i] - array[i+1]) /
                       np.abs(array[i-1] - array[i]))
